...
void boardInit(void){
    ...
    stmFlsh_tmr = &tmr[tmrIndx++];  // register timer for stm_flash thread
    ...
    printS("setup adc...");
    ADC_Setup(&adcD, &hadc1, &tmr[tmrIndx++], usrRead, usrWrite, 0);   // setup ADC work thread
    printS("ok\r\n");
    ...
    log("<%s [0].ret:%d >", __func__, ret);    
    
    log("setup cw2217[1]..");
    ret = cw2217[1].init(&cw2217[1].rsrc);
    log("<%s [1].ret:%d >", __func__, ret);
